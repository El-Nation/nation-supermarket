...
export const getAllPayments = async (req: Request, res: Response) => {
    try {
        const query = await pool.query(`SELECT id, order_id, payment_reference, amount, payment_status, gateway_reference, payment_date as created_at FROM payments ORDER BY payment_date DESC`);
        res.json(query.rows);
