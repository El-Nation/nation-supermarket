...
export default function ManageOrders() {
    ...
    const fetchOrders = async (query: string = '') => {
        try {
            setLoading(true);
            const res = await api.get(`/admin/orders${query ? `?search=${encodeURIComponent(query)}` : ''}`);
                setOrders(res.data);
            } catch(e) {
        ...
        <div style={{maxWidth: 1200, color: '#1e293b'}}>
            ...
            <div className="admin-card" style={{padding: '0', overflow: 'hidden', border: '1px solid #e2e8f0', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.05)'}}>
                <div style={{padding: '1.5rem 2rem', backgroundColor: '#f8fafc', borderBottom: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem'}}>
                    ...
                    <form onSubmit={(e) => { e.preventDefault(); fetchOrders(searchQuery); }} style={{ display: 'flex', gap: '0.5rem' }}>
                        <input 
                            type="text" 
                            className="form-input" 
                            placeholder="Search order ref, name, email..." 
                            value={searchQuery}
                        ...
                        {searchQuery && (
                            <button type="button" onClick={() => { setSearchQuery(''); fetchOrders(''); }} className="admin-btn-primary" style={{ backgroundColor: '#e2e8f0', color: '#0f172a', padding: '0.5rem 1rem' }}>Clear</button>
                        )}
                    </form>
                </div>
                            ...
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={6} style={{textAlign: 'center', color: '#94a3b8', padding: '5rem 2rem'}}>
                                        ...
                                        {searchQuery ? 'No results found.' : 'No active orders generated natively yet. Create one via Checkout Simulator.'}
                                    </td>
                                </tr>
                            ) : orders.map((o, i) => {
